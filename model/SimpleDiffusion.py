...
class CondGaussianDiffusion(GaussianDiffusion):
    ...
    def forward(self, img, cond_img, depth_img, seg=None, extra_cond=None, *args, **kwargs):
        b, channels, h, w = img.shape
        cond_channels = cond_img.shape[1]
        assert channels == self.channels
        assert h == w == self.image_size
        assert cond_channels == self.cond_channels
        assert depth_img.shape[1] == self.depth_channels
        ...
        return self.p_losses(img, times, cond_img, depth_img, seg, extra_cond, *args, **kwargs)

    def p_losses(self, x_start, times, cond_img, depth_img, seg=None, extra_cond=None, noise=None, *args, **kwargs):
        ...
        model_out = self.model(torch.cat([x, extra_cond], dim=1), log_snr, cond_img, depth_img)
