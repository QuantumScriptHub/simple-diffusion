...
class PolypObjDataset(data.Dataset):
    def __init__(self, image_root, gt_root, depth_root, trainsize, mean=None, std=None, randomPeper=False,
    ...
    def __getitem__(self, index):
        data = {}
        # read imgs/gts/depths
        image = self.rgb_loader(self.images[index])
        gt = self.binary_loader(self.gts[index])
        gt = np.expand_dims(np.asarray(gt), axis=2)
        ...
        # data augmentation
        image, gt_depth = self.aug_transform(image=np.asarray(image), masks=[gt, depth]).values()
        image, gt_depth = albumentations.PadIfNeeded(*image_size[::-1], border_mode=0)(image=image, masks=[gt_depth[0], gt_depth[1]]).values()
        image, gt_depth = albumentations.RandomCrop(*image_size[::-1])(image=image, masks=[gt_depth[0], gt_depth[1]]).values()
        gt, depth = gt_depth[0][:, :, 0], gt_depth[1]
        ...
        depth = self.depth_transform(Image.fromarray(depth))
        data['image'] = image
        data['gt'] = gt
        data['depth'] = depth
    ...
    def filter_files(self):
        assert len(self.images) == len(self.gts) and len(self.depths) == len(self.gts)
        images = []
        gts = []
        depths = []
...
def get_loader(image_root, gt_root, depth_root, batchsize, trainsize,
               shuffle=True, num_workers=12, pin_memory=True):
    dataset = PolypObjDataset(image_root, gt_root, depth_root, trainsize)
...
class test_dataset(data.Dataset):
    ...
    def load_data(self):
        ...
        return image, gt, depth, name, image_for_post
    ...
    def __getitem__(self, item):
        ...
        depth = self.rgb_loader(self.depths[item])
        ...
        return {'image': image, 'gt': gt, 'depth': depth, 'name': name, 'image_for_post': image_for_post}
